...
We will not always adhere to the convention of using initial English letters for constant symbols and final English letters for variables symbols; this will only be a heuristic and how any given letter is used should be understood from the context. 
...
A \textbf{(domain guarded) function symbol} $f$ with \textbf{domain condition} $D$ is a symbol that takes in a term $t$ for which a proof of $D(t)$ exists and returns new term $f(t)$ that refers to an existing object in the domain of discourse. 
...
Whenever we use a domain guarded function symbol, the relevant domain condition must be already available in the context.
...
\begin{prop}\label{ch1thm_fn_inverses}
...
\begin{proof}
	...
	If $y\not\in\im f$, then $\langle y, a_{0}\rangle$ satisfies the second disjunct and $\langle y, a_{0}\rangle\in G_{g}$.
	...
	If $y\in\im f$, the second disjunct fails for both pairs, so $\langle x, y\rangle, \langle x', y\rangle\in\textrm{Graph}(f)$, i.e. $f(x) = y = f(x')$, and injectivity gives $x = x'$.
	If $y\not\in\im f$, the first disjunct fails for both pairs, so $x = a_{0} = x'$.
	In both cases, $x = x'$.
	
	Thus $g:B\rightarrow A$ is a function with $g(y) = x$ whenever $f(x) = y$.
	Given any $x\in A$, take $y = f(x)$, so then $y\in\im f$ and the unique element of $A$ mapping to $y$ under $f$ is $x$ itself, so
	...
	For any $a\in A$, we have $g(b) = a$ for $b = f(a)$, so $g$ is surjective. 
