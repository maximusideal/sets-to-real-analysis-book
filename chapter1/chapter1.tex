...
\section{Foundations and Framework}\label{ch1sec_foundations_and_framework}
...
A \textbf{one-place predicate} $P$ is anything that takes in an object $a$ and outputs a proposition $P(a)$. 
For example, if $P(a)$ means, ``$a$ went to the store," then $P$ refers to ``went to the store."
...
A \textbf{two-place predicate} $P$ takes objects $a, b$ to form a proposition $P(a, b)$ and a \textbf{three-place predicate} $P$ takes objects $a, b, c$ to form a proposition $P(a, b, c)$.
For example, if $P(a, b)$ means, ``$a$ is on top of $b$," then $P$ refers to ``is on top of."
...
Inside propositions with quantifiers, we use \textbf{variable symbols} like $x, y, z$, which do not refer to any specific object but are placeholders for arbitrary objects in a given domain of discourse.
In general, a \textbf{bound variable} is a variable whose occurrence lies within the scope of something (a binding operator) that declares the variable role.
The binding operators can be quantifiers, but later in the text we will come across integral, summation, and limit notations that also serve as binding operators. 
Bound variables act as local placeholders whose names may be changed without altering the meaning of the expression.
A variable that is not bound is called a \textbf{free variable}. 

In contrast to variables, we use \textbf{constant symbols} like $a, b, c$ to refer to some specific objects in the domain of discourse, even if we might not know exactly which object. 
We will not always adhere to the convention of using initial English letters for constant symbols and final English letters for variable symbols; this will only be a heuristic and how any given letter is used should be understood from the context. 

When we deal solely with propositions without predicates or quantifiers, we are dealing with \textbf{propositional logic}.
...
In mathematics, \textbf{terms} are expressions that designate objects in a given domain of discourse.
If two terms, say $a$ and $b$, refer to the same object, we write $a = b$. 
If the terms $a$ and $b$ refer to different objects, we write $a\ne b$ as a shorthand for $\neg (a = b)$. 
...
\noindent The \textbf{indiscernibility of identicals} is also called the \textbf{substitution property}.
In the way we formulated it, the indiscernibility of identicals is a schema rather than a single statement, so it represents multiple statements, one for each predicate $P$. 

We will be flexible with our notation in obvious ways.
For example, by writing $a = b = c$, we mean that $a = b$ and $b = c$ hold simultaneously. 
...
This asserts not only that an object $a$ satisfies $P$, but also that the object is unique, so if $b$ also satisfies $P$, then $a = b$.
We may abbreviate this as 
\[ \exists ! x\, P(x), \] 
which is a statement of unique existence. 

\subsection{Constant, (Domain Guarded) Function, and Relation Symbols}
...
A constant symbol $c$ (already introduced above) is a symbol that refers to a fixed existing object in the domain of discourse. 
A \textbf{(domain guarded) function symbol} $f$ with \textbf{domain condition} $D$ is a symbol that takes in a term $t$ for which a proof of $D(t)$ exists and returns a new term $f(t)$ that refers to an existing object in the domain of discourse. 
In some cases, the domain condition $D$ may be a two-place predicate and the function symbol $f$ may take two terms $s, t$ for which a proof of $D(s, t)$ exists to produce a new term $f(s, t)$. 
...
We deviate away from usual first-order logic because of our domain guarded scheme taken from Wiedijk and Zwanenburg's work. % cite First order logic with domain conditions by W&Z
This is so that certain junk terms like $1/0$ (division by zero) remain genuinely undefined.
Whenever we use a domain guarded function symbol, the relevant domain condition must already be available in the context.
This has the consequence that propositions and predicates must be read from left to right such that the well-formedness of terms crucially depends on what is established to the left of the term and in the given context. 

\begin{example}
	Suppose our domain of discourse is the set of rational numbers.
	We haven't yet introduced rational numbers formally, but we use them here for illustrative purposes. 
	Suppose division $d(x, y) = x/y$ is domain guarded by $D(x, y)\equiv y\ne 0$. 
	
	Consider the predicate
	\[ P(x)\equiv (x\ne 0)\land (1/x > 0). \]
	If $x\ne 0$, the first clause is true and $D(1, x)$ holds, so then $1/x$ is well-formed and the second clause may be evaluated as true or false.
	If $x = 0$, the first clause is false, and we stop there immediately to conclude $P(x)$ is false without reading the second clause.
	Consider the predicate 
	\[ Q(x)\equiv (1/x > 0)\land (x\ne 0). \]
	Here, nothing ensures $x\ne 0$ when reading left to right, so no proof of $D(1, x)$ exists, and so $Q(x)$ is not well-formed.
	
	The proposition
	\[ R\equiv \exists x\, (x\in\mathbb{Q}\land 1/x = 1) \]
	is not well-formed because nothing ensures $x\ne 0$ to the left of $1/x$, so no proof of $D(1, x)$ exists.
	However,
	\[ S\equiv \exists x\, (x\in\mathbb{Q}\land x\ne 0\land 1/x = 1) \]
	is well-formed, because the $x\ne 0$ clause proves $D(1, x)$ before we get to $1/x$. 
\end{example}

\subsection{Axioms, Theory, and Models}

Mathematics deals with structures such as numbers, shapes, graphs, spaces, and more complicated systems.
These structures sit on the \textbf{semantic} side of mathematics.
In order to reason about such structures, we write statements in a language.
This language, together with the rules for forming statements and proofs done using logic, sits on the \textbf{syntactic} side of mathematics.

An \textbf{axiom} is a proposition we take as true within a given theory.
Axioms in a given theory are used as the starting point so that we may prove further statements called theorems. 
Now if the axioms of a given theory are true about some mathematical structure(s), then by soundness of logic all the resulting theorems should also be true about the mathematical structure(s). 

An \textbf{axiom schema} is a rule for generating axioms such that for each appropriate predicate $P$, the schema yields a unique proposition by having $P$ substituted in the right places.
It is simply a way to represent multiple axioms, one for each predicate. 

\begin{example}\label{ch1ex_axioms_triang_NagelNewman}
	This example is adapted from Nagel and Newman. % cite Godel's Proof by Nagel & Newman
	We take the formal language of two sorts $K$ and $L$ where variables $x, y, z, w$ belong to $K$ and variables $i, j, k, l$ belong to $L$, and there is a single primitive relation symbol $\propto$.
	We read $x\propto i$ as ``$x$ is blorged in $i$.''
	The theory consists of the following axioms:
	\begin{enumerate}[itemsep=-0.4ex, label=\textup{(T\arabic*)}]
		\item Any two distinct $K$-objects are blorged in exactly one $L$-object:
		\[ \forall x\forall y\, (x\ne y\implies\exists ! i\, (x\propto i\land y\propto i)). \]
		\item No $K$-object is blorged in more than two $L$-objects:
		\[ \neg\exists x\exists i\exists j\exists k\,(i\ne j\land i\ne k\land j\ne k\land x\propto i\land x\propto j\land x\propto k). \]
		\item The $K$-objects are not all blorged in a single $L$-object:
		\[ \neg\exists i\,\forall x\,(x\propto i). \]
		\item Any two distinct $L$-objects blorg exactly one $K$-object:
		\[ \forall i\forall j\,(i\ne j\implies\exists!x\,(x\propto i\land x\propto j)). \]
		\item No $L$-object blorgs more than two $K$-objects:
		\[ \neg\exists i\exists x\exists y\exists z\,(x\ne y\land x\ne z\land y\ne z\land x\propto i\land y\propto i\land z\propto i). \]
		\item There exist two distinct $K$-objects:
		\[ \exists x\exists y\, (x\ne y). \]
	\end{enumerate}
	
	Using these starting axioms of our theory, we can use logical inference to prove various theorems. 
	For example, we may deduce that there must be exactly three $K$-objects, and all this can be done without having to interpret what any of the words or symbols mean. 
	
	However, the axioms we laid out happen to be true statements about a real mathematical object.
	In our example, we characterized a triangle-like structure. 
	We may interpret $K$-objects as vertices, $L$-objects as line segments or edges, and the blorg relation $\propto$ to mean that a vertex is part of the line segment or edge. 
	Since (T1) to (T6) are true of triangles, any deductions from them such as the theorem that there are exactly three $K$-objects are also true of triangles. 
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[
			...
			%--- triangle sides / L-objects ---
			\draw[-] (a) -- node[above] {$I_{L}$} (b);
			\draw[-] (a) -- node[right] {$J_{L}$} (c);
			\draw[-] (b) -- node[below] {$K_{L}$} (c);
			...
			%--- vertex labels ---
			\node[above=4pt] at (a) {$X_{K}$};
			\node[left=6pt] at (b) {$Y_{K}$};
			\node[right=6pt] at (c) {$Z_{K}$};
		\end{tikzpicture}
		\caption{Triangle model of the axioms.}
	\end{figure}
\end{example}

Axiom lists may characterize a class of structures. 
Sometimes they may narrow the class down to exactly one structure, but this does not have to be true. 
For example, if we removed some axioms from \Cref{ch1ex_axioms_triang_NagelNewman}, then the triangle would not be the only compatible structure satisfying the axioms. 

\subsection{Extending Our Language and Theory}

In our text, we will be extending our language and theory as our text unfolds exactly as in real life mathematical practice. 
Let us explain what this means. 
Let $L$ be a \textbf{language} containing logical symbols along with constant, function, and relation symbols, and let $T$ be a \textbf{theory-as-a-set-of-axioms} in $L$. 
We say $L'$ \textbf{extends} $L$ if every symbol of $L$ is also a symbol of $L'$.
We say $T'$ \textbf{extends} $T$ if every axiom of $T$ is also an axiom of $T'$.

In this text, extensions arise by adding new constant, function, or relation symbols, together with axioms governing them. 
However, we require that every extension $T'$ in an expanded language $L'$ is \textbf{conservative} over $T$, meaning that every proposition expressible in the original language $L$ that is provable from $T'$ must be provable from the original theory $T$.
This way, what is and is not entailed by our original axioms is preserved. 
We do not want to add symbols and axioms that break conservativity, because then the extension would leave us with a theory that is inequivalent to the original theory.
In turn that would change what mathematical structures we are talking about.

Consider the following scenario.
If our initial theory proves $\neg\exists x\, P(x)$, then adding a new constant symbol $c$ such that $P(c)$ holds introduces a contradiction, because the existence of $c$ implies $\exists x\, P(x)$.
This breaks not only conservativity over the original theory but consistency itself. 
In this case, the expansion of language and the introduction of new notation is not harmless. 
Let us lay out the precise cases where expansion of language and theory is conservative. 
...
\begin{itemize}[itemsep=-0.4ex]
	...
	\begin{align}\label{ch1eqn_const_ax}
		P(c).
	\end{align}
	...
	\end{align*}
	for some one-place predicate $D$ and two-place predicate $P$, then given $x$ such that $D(x)$ holds we want to be able to pick out $y$ such that $P(x, y)$ holds. 
	We do this by introducing a new symbol $f$ with domain condition $D$ and a new axiom 
	\begin{align}\label{ch1eqn_fn_ax}
		\forall x\, (D(x)\implies P(x, f(x))).
	\end{align}
	Since uniqueness of outputs must be established, if $c, d$ are such that $D(c)$ and $P(c, d)$, it must follow that $d = f(c)$. 
	...
	\begin{align}\label{ch1eqn_rel_ax}
		\forall x\forall y\, (P(x, y) \iff xRy).
	\end{align}
\end{itemize}

All three cases are \textbf{extensions by definition}.
In an extension by definition, any individual formula in the expanded language $L'$ can be translated into a provably equivalent formula in the old language $L$. 
The translation is done on the atomic formulas and terms containing the new symbols. 
For example, if $\exists ! x\, P(x)$ holds and $c$ is a constant symbol such that \Cref{ch1eqn_const_ax} holds, then in any formula we may replace every atomic formula of the form $Q(c)$ with $\exists x\, (P(x)\land Q(x))$ to get an equivalent formula.
If a function symbol $f$ is introduced under \Cref{ch1eqn_fn_ax} with uniqueness of outputs, then terms involving $f$ are eliminated from the innermost outward:
At each step, we rewrite each atomic formula $Q(f(t))$ with $t$ not containing $f$ as $\exists y\, (P(t, y)\land Q(y))$, provided $D(t)$ holds. 
By repeating this and working outwards, every occurrence of $f$ is removed. 
If a relation $R$ is introduced by \Cref{ch1eqn_rel_ax}, then every occurrence of $sRt$ may be replaced by $P(s, t)$ where $s, t$ are terms. 
Consequently, any proof involving $c$, $f$, or $R$ introduced by extension by definition may be rewritten without those symbols. 
This guarantees the extension $T'$ of $T$ is conservative.

There are other ways in which constant symbols are introduced, but they are only done in \emph{local contexts} (like in a proof or a theorem statement) and the \emph{global language} does not change.
Let us go over ways this happens in proofs. 

\begin{itemize}[itemsep=-0.4ex]
	\item\textsc{Universal generalization.}
	Suppose we want to prove
	\[ \forall x\, P(x) \]
	for some predicate $P$.
	To prove this, we would introduce a constant symbol $c$ with nothing assumed other than background information like the axioms of our theory.
	After that, we use the axioms and logical inference to deduce $P(c)$.
	Because the object $c$ refers to is arbitrary, we generalize, and infer $\forall x\, P(x)$, with $c$ no longer part of our vocabulary. 
	As an instructive example, suppose $P(x)$ is an implication of the form $R(x)\implies S(x)$. 
	We introduce $c$ and consider two cases.
	If $c$ does not satisfy $R$, then $R(c)\implies S(c)$ is true by virtue of the antecedent being false. 
	If $c$ satisfies $R$, then we use axioms and logical inference to deduce that $c$ satisfies $S$, so that $R(c)\implies S(c)$ is true by virtue of the consequent being true. 
	Since this applies to all $c$, we generalize and conclude $\forall x\, (R(x)\implies S(x))$, with $c$ no longer part of our vocabulary. 
	\item\textsc{Existential instantiation.}
	Suppose we want to prove some proposition $Q$ using a previously established fact that $\exists x\, P(x)$ where $P$ is some predicate. 
	To proceed, we introduce a constant symbol $c$ such that $P(c)$, do reasoning in the given context, reach conclusion $Q$, and then discharge $c$.
	The constant symbol $c$ must not appear anywhere before its introduction or in $Q$.
	It is important to stress that $c$ is a \emph{generic witness} to the existential statement $\exists x\, P(x)$. 
	This means the only fact we may use about $c$ is that it satisfies $P$ and anything entailed by that, even though a \emph{specific object} might satisfy many more predicates. 
\end{itemize}

\noindent In these local contexts, we do not need uniqueness conditions.
For example, introducing a witness $c$ such that $P(c)$ can be done if $\exists x\, P(x)$ is established with uniqueness not necessarily established (so if $d$ is also such that $P(d)$ it does not necessarily follow that $c = d$). 
Proofs that invoke temporary constant symbols are translatable into proofs that avoid this technique.
However, individual sentences with $c$ cannot be translated into equivalent sentences without $c$ unless the uniqueness condition $\exists ! x\, P(x)$ also holds.
(Proving all these facts is outside the scope of this text.)
This is why we limit changes of our global language to only cases where uniqueness conditions hold. 

There are formal proof systems that make all this much more precise.
In some systems like natural deduction, local introductions of new constant symbols are part of the proof-theoretic framework, but there are systems like Hilbert's system that avoid any introduction of constant symbols altogether.
The price to pay in systems like Hilbert's system is that proofs are much more arduous and difficult to follow. 
...
More generally, it can be any atomic piece of a writing system that can't be broken down further. 
...
The base formal language of ZFC consists of the logical symbols and one additional relation symbol $\in$.
...
\begin{enumerate}[itemsep=-0.4ex, label=\textup{(ZFC${\arabic*}$)}]\setcounter{enumi}{2}
	...
	Given sets $A, c$ and a (one-place or two-place) predicate $P$, there exists a set 
	\[ B = \{ x\in A \,|\, P(x, c) \} \] 
	such that $x\in B$ if and only if $x\in A$ and $P(x, c)$. 
	Intuitively, $B$ is the subset of $A$ consisting of elements that have the property $P$ (with parameter $c$), and we call this the \textbf{specified subset} of $A$ of elements satisfying predicate $P$ (with parameter $c$).
...
The axiom schema of separation depends on predicates defined in the original language consisting only of the non-logical $\in$ symbol.
However, because all expansions of our language will be extensions by definition (as explained in \Cref{ch1sec_foundations_and_framework}), we may invoke predicates that use the expanded language without altering the theory whatsoever. 
...
\begin{proof}
	...
	Let $P(x)\equiv x\notin x$ and apply separation to get the subset $B = \{x\in A \,|\, x\notin x\}$.
	If $B\in B$, then the definition of $B$ forces $B\notin B$.
	If $B\notin B$, then the definition of $B$ forces $B\in B$.
...
If $A$ is a set such that $\forall x\, (x\in A\iff x = a)$, we call $A$ a \textbf{singleton}, and the axiom of extensionality (ZFC2) implies $A = \{a, a\}$, which is unique with respect to $a$. 
...
If $\bigcap\varnothing$ were defined such that the subset property held, then the fact that $\varnothing\subseteq A$ for all sets $A$ would imply $\bigcap\varnothing$ contains all sets.
...
If $a\ne b$, then $\bigcup\langle a, b\rangle \ne \bigcap\langle a, b\rangle$ so $P(x, \langle a, b\rangle)$ is true only for $x\notin\bigcap\langle a, b\rangle = \{a\}$, i.e. $x = b$, so the set in the set-builder notation is $\{ x\in\{a, b\} \,|\, x=b \} = \{b\}$ and thus the left-hand side evaluates to $b$. 
Consequently, 
...
\begin{definition}
	...
	If $\langle a, b\rangle\notin G_{R}$, then we say \textbf{$a$ is not related to $b$ by relation $R$}, and we write $a \not\!\! R\, b$.
\end{definition}
...
\begin{definition}
	...
	\begin{itemize}[itemsep=-0.4ex]
		\item If an element $b\in A$ is such that $\forall x\in X\, (x\le b)$, we say $b$ is an \textbf{upper bound} of $X$. 
		If an element $b\in A$ is such that $\forall x\in X\, (x < b)$, we say $b$ is a \textbf{strict upper bound} of $X$.
		If an upper bound of $X$ exists, we say $X$ is \textbf{bounded above}. 
		\item If an element $b\in A$ is such that $\forall x\in X\, (x\ge b)$, we say $b$ is an \textbf{lower bound} of $X$. 
		If an element $b\in A$ is such that $\forall x\in X\, (x > b)$, we say $b$ is a \textbf{strict lower bound} of $X$.
		If an upper bound of $X$ exists, we say $X$ is \textbf{bounded below}. 
	\end{itemize}
	...
	\begin{itemize}[itemsep=-0.4ex]
		\item If $m\in X$ such that $\forall x\in X\, (m\le x\implies x = m)$, then $m$ is called a \textbf{maximal} element of $X$. 
		\item If $m\in X$ such that $\forall x\in X\, (x\le m\implies x = m)$, then $m$ is called a \textbf{minimal} element of $X$. 
	\end{itemize}
\end{definition}
...
\begin{example}\label{ch1ex_maximalelems}
	...
	This gives us a concrete example where the concepts of maximal and greatest differ. 
	\begin{figure}[H]
		\centering
		\begin{tikzpicture}[
			>=stealth,
...
\[ \{ a'\in A \,|\, a'\sim a \}, \]
which we denote as $[a]_{\sim}$ or usually just $[a]$.
...
\section{Set-Coded Functions}\label{ch1sec_fns}

\subsection{Basic Definitions}

Based on the axioms telling us what is allowed and not allowed in set theory, we introduced various function-like operations like the power set operation $\cc{P}$, the union operation $\bigcup$, and the intersection operator $\bigcap$.
...
\begin{definition}
	...
	\begin{itemize}[itemsep=-0.4ex]
		\item the \textbf{domain} is $A = \textrm{Dom}(f)$,
		\item the \textbf{codomain} is $B = \textrm{Cod}(f)$,
		\item the \textbf{graph} is $G_{f} = \textrm{Graph}(f)$. 
	\end{itemize}
	We denote the fact that $f$ is a function form $A$ to $B$ by writing $f:A\rightarrow B$. 
\end{definition}
...
\begin{definition}
	...
	If $f$ is a partial function, we define the following:
	\begin{itemize}[itemsep=-0.4ex]
		\item the \textbf{ambient domain} is $A = \textrm{AmbDom}(f)$,
		\item the \textbf{active domain} is $S = \textrm{ActDom}(f)$ defined as the set of $x\in A$ such that $\langle x, y\rangle\in G_{f}$ for some $y\in B$,
		\item the \textbf{codomain} is $B = \textrm{Cod}(f)$,
		\item the \textbf{graph} is $G_{f} = \textrm{Graph}(f)$.
	\end{itemize}
...
In our setup of terminology, a (set-coded) function is a partial function where the active domain is the ambient domain.
We will oftentimes refer to a function as a \textbf{map} or \textbf{mapping}. 

We think of a partial function as ``sending" elements of the active domain to elements of the codomain.
...
\[ D(f, x)\equiv (f\text{ is a partial function})\land x\in\textrm{ActDom}(f), \]
and let $P$ be the predicate
\[ P(f, x, y)\equiv D(f, x)\land \langle x, y\rangle\in\textrm{Graph}(f). \]
We leave it to the reader to verify 
\[ \forall f\forall x\, (D(f, x)\implies \exists ! y\, P(f, x, y)). \]
Given this, we define a function symbol $\textrm{App}$ with domain condition $D$ such that $\textrm{App}(f, x)$ is the unique $y$ for which $P(f, x, y)$ holds, i.e. $\langle x, y\rangle\in\textrm{Graph}(f)$.
...
\[ D(f, E)\equiv (f\text{ is a partial function})\land E\subseteq\textrm{AmbDom}(f), \]
and let $P$ be the predicate
\begin{align*}
	P(y, c) &\equiv (c\text{ is a Kuratowski ordered pair}) \\
	&\quad\land (\pi_{L}(c)\text{ is a partial function}) \\
	&\quad\land \pi_{R}(c)\subseteq\textrm{AmbDom}(\pi_{L}(c)) \\
	&\quad\land \exists x\, (x\in\pi_{R}(c)\cap\textrm{ActDom}(\pi_{L}(c))\land y = \pi_{L}(c)(x)),
\end{align*}
which says $c$ is a Kuratowski ordered pair consisting of a partial function $f$ and a subset $E$ of the ambient domain of $f$ such that $y = f(x)$ for some $x\in E$.
Then for all $f, E$ such that $D(f, E)$ holds, by separation there exists a unique set of the form
\[ \textrm{Sep}_{P}(\textrm{Cod}(f), \langle f, E\rangle), \]
where $\textrm{Sep}_{P}(A, c)$ is the subset of $A$ of elements $y$ satisfying $P(y, c)$.
We label this as $\textrm{AppIm}(f, E)$, so $\textrm{AppIm}$ is defined for all $f, E$ for which $D(f, E)$ holds. 
...
\begin{definition}
	...
	\begin{itemize}[itemsep=-0.4ex]
		...
		We show below that if it exists, then it is unique.
		We denote it by $f^{-1}$, which can be done due to uniqueness. 
	\end{itemize}
	If an inverse of $f$ exists, $f$ is called \textbf{invertible}. 
\end{definition}

\begin{prop}\label{ch1thm_fn_inv_unique}
	If $f:A\rightarrow B$ is an invertible function, then its left-inverse and right-inverse are unique and the left-inverse and right-inverse are equal to each other. 
\end{prop}
\begin{proof}
	Let $f:A\rightarrow B$ be an invertible function and let $g:B\rightarrow A$ be its inverse. 
	Suppose $g_{L}:B\rightarrow A$ is a left-inverse of $f$.
	For any $y\in B$, we have $f(g(y)) = y$ because $g$ is an inverse of $f$, so $x = g(y)$ is the element such that $f(x) = y$.
	Then because $g, g_{L}$ are left-inverses of $f$, 
	\[ g_{L}(f(x)) = x = g(f(x)), \quad\text{so}\quad g_{L}(y) = g(y). \]
	By \Cref{ch1thm_lemma_equality_of_fns}, $g_{L} = g$, which means all left-inverses of $f$ are $g$. 
	
	Suppose $g_{R}:B\rightarrow A$ is a right-inverse of $f$.
	For any $y\in B$, we have $f(g(y)) = y = f(g_{R}(y))$ because $g, g_{R}$ are right-inverses of $f$, so
	\[ g(f(g(y))) = g(f(g_{R}(y))), \quad\text{so}\quad g(y) = g_{R}(y), \]
	because $g$ is an inverse of $f$. 
	By \Cref{ch1thm_lemma_equality_of_fns}, $g_{R} = g$, which means all right-inverses of $f$ are $g$.
	
	Lastly, for any left-inverse $g_{L}$ and right-inverse $g_{R}$ of $f$, both are equal to $g$, so $g_{L} = g_{R}$. 
\end{proof}
...
\begin{example}[Inverse]
	...
	Then
	\begin{align*}
		g(f(a)) = a, &\qquad& g(f(b)) = b, &\qquad& g(f(c)) = c, \\
		f(g(x)) = x, &\qquad& f(g(y)) = y, &\qquad& f(g(z)) = z,
	\end{align*}
	so $g\circ f = \id_A$ and $f\circ g = \id_B$.
...
\begin{example}[$f$ is not well-defined]\label{ch1ex_f_not_well_defd}
	Let $A = \{a, b, c, d, e, i\}$ and $B = \{\text{Consonant}, \text{Vowel}\}$ where all elements listed are distinct from one another.
	Let $P(t)\equiv t = a\lor t = e\lor t = i$.
	Define a relation $\sim$ on $A$ by
	\begin{align*}
		a\sim a, \qquad b\sim b, \qquad c\sim c, \qquad d\sim d, \qquad e\sim e, \qquad i\sim i, \\
		a\sim b, \qquad b\sim a, \qquad c\sim d, \qquad d\sim c, \qquad e\sim i, \qquad i\sim e.
	\end{align*}
	...
	\begin{align*}
		f([x]) = \begin{cases}
			\text{Vowel} &\text{ if } P(x), \\
			\text{Consonant} &\text{ otherwise}.
		\end{cases}
	\end{align*}
	This function is not well-defined!
	The problem is that because $[a] = [b]$, it must be the case that $f([a]) = f([b])$, but if we follow our naive rule then $f([a]) = \text{Vowel}$ and $f([b]) = \text{Consonant}$.
	
	On one hand, $f([a]) = f([b])$ but we declared at the start that $\text{Consonant}$ and $\text{Vowel}$ are distinct elements: $\text{Consonant}\ne \text{Vowel}$. 
...
where
\[ P(t, \sim)\equiv (\sim\text{ is an equivalence relation})\land \exists a\, (a\in\textrm{Dom}(\sim) \land t = \langle a, [a]_{\sim}\rangle). \]
...
\begin{proof}
	...
	\begin{align*}
		...
		&\quad\land \exists a\, (a\in\textrm{Dom}(\pi_{L}(c))\land t = \langle [a]_{\pi_{R}(c)}, \pi_{L}(c)(a)\rangle ). 
	\end{align*}
...
\begin{example}
	Let $A = \{a, b, c, \delta, \varepsilon\}$ and $B = \{\textrm{Latin}, \textrm{Greek}\}$ where all elements listed are distinct. 
	...
	\begin{align*}
		...
		& \delta\sim\delta, \quad \varepsilon\sim\varepsilon, \quad \delta\sim \varepsilon, \quad \varepsilon\sim \delta.
	\end{align*}
	We leave it to the reader to check this is an equivalence relation on $A$.
	...
	More formally, this is $G_{f} = \{ t\in A\times_{K} B \,|\, Q(t) \}$ where
	...
	Now because 
	\[ f(a) = f(b) = f(c) = \textrm{Latin} \quad\text{and}\quad f(\delta) = f(\varepsilon) = \textrm{Greek}, \] 
	the relationship $x\sim x'$ implies $f(x) = f(x')$ for all $x, x'\in A$, and so \Cref{ch1thm_quotient_factor_thm} is applicable. 
	Thus, there exists a unique function $g:A/\sim\;\rightarrow B$ such that $f = g\circ q$ where $q$ is the quotient map. 
	In this case, 
	\[ g(\{a, b, c\}) = \textrm{Latin} \quad\text{and}\quad g(\{\delta, \varepsilon\}) = \textrm{Greek}. \]
...
\section{Choice Principles}\label{ch1sec_choice}
...
Any function matching the description of $f$ in the axiom of choice is called a \textbf{choice function}.
The meaning of such a function is that for each $a\in A$, the function picks out some element of $a$. 
A very important fact about the axiom of choice is that it is making an assertion about arbitrary collections of nonempty sets.
Let us go over an application of the axiom of choice.

\begin{prop}\label{ch1thm_fn_inverses_vs_ject}
	Let $f:A\rightarrow B$ be a function whose domain $A$ is nonempty. 
	Then the following statements are true:
	\begin{enumerate}[itemsep=-0.4ex, label=\textup{(\alph*)}]
		\item $f$ is injective if and only if it has a left-inverse.
		Every left-inverse, if any exists, is surjective.
		\item $f$ is surjective if and only if it has a right-inverse.
		Every right-inverse, if any exists, is injective.
		\item $f$ is bijective if and only if it has an inverse.
		The inverse, if it exists, is bijective. 
	\end{enumerate}
\end{prop}
\begin{remark}
	Part (b) of the proposition specifically requires the axiom of choice. 
	In the ending problem set, we claim that (b) is actually equivalent to the axiom of choice. 
	Recall from \Cref{ch1thm_fn_inv_unique} that the inverse of $f$, if it exists, is unique. 
\end{remark}
\begin{proof}
	(a).
	Suppose $f$ is injective.
	Since $A$ is nonempty, we may take an arbitrary element $a_{0}\in A$.
	We define $g:B\rightarrow A$ by
	\[ g(y) = \begin{cases}
		x &\text{ if } y\in\im f, \text{ where $x$ is the unique element with } f(x)=y, \\
		a_{0} &\text{ if } y\notin\im f.
	\end{cases} \]
	The element $x$ in the first case is unique precisely because $f$ is injective.
	To justify existence, apply separation to $B\times_{K} A$ to obtain
	\[ G_{g} = \{ o\in B\times_{K} A \,|\, P(o, \langle f, a_{0}\rangle) \} \]
	where
	\begin{align*}
		P(o, c) &\equiv (o, c\text{ are Kuratowski ordered pair})\land (\pi_{L}(c)\text{ is a function}) \\
		&\quad\land ( \underbrace{ \langle\pi_{R}(o), \pi_{L}(o)\rangle\in\textrm{Graph}(\pi_{L}(c)) }_{\text{first disjunct}}\lor \underbrace{ (\pi_{L}(o)\notin\im(\pi_{L}(c))\land \pi_{R}(o) = \pi_{R}(c)) }_{\text{second disjunct}} ).
	\end{align*}
	Reading $o = \langle y, x\rangle$, the predicate says either $f(x) = y$, or $y\notin\im f$ and $x = a_{0}$.
	Take $g = \langle G_{g}, \langle B, A\rangle\rangle$, which exists.
	Since $G_{g}\subseteq B\times_{K} A$, it remains to verify (F1) and (F2) from the definition of functions.
	
	For (F1), let $y\in B$.
	If $y\in\im f$, there exists $x\in A$ with $f(x) = y$, so $\langle x, y\rangle\in\textrm{Graph}(f)$, and so $\langle y, x\rangle$ satisfies the first disjunct and $\langle y, x\rangle\in G_{g}$.
	Else if $y\notin\im f$, then $\langle y, a_{0}\rangle$ satisfies the second disjunct and $\langle y, a_{0}\rangle\in G_{g}$.
	In both cases, there exists $x\in A$ such that $\langle y, x\rangle\in G_{g}$.
	
	For (F2), suppose $\langle y, x\rangle, \langle y, x'\rangle\in G_{g}$.
	If $y\in\im f$, the second disjunct fails for both pairs, so $\langle x, y\rangle, \langle x', y\rangle\in\textrm{Graph}(f)$, i.e. $f(x) = y = f(x')$, and injectivity of $f$ gives $x = x'$.
	Else if $y\notin\im f$, the first disjunct fails for both pairs, so $x = a_{0} = x'$.
	In both cases, $x = x'$.
	
	Thus, $g:B\rightarrow A$ is a function with $g(y) = x$ whenever $f(x) = y$.
	Given any $x\in A$, take $y = f(x)$, so then $y\in\im f$ and the unique element of $A$ mapping to $y$ under $f$ is $x$, so
	\[ g\circ f(x) = g(f(x)) = g(y) = x. \]
	Hence $g\circ f = \id_{A}$ by \Cref{ch1thm_lemma_equality_of_fns}, and $g$ is a left-inverse of $f$.
	
	Conversely, suppose $f$ has a left-inverse $g:B\rightarrow A$. 
	Then if $x, x'\in A$ such that $f(x) = f(x')$, we find that by applying $g$ to both sides, we get $g(f(x)) = g(f(x'))$, so
	\[ x = g\circ f(x) = g(f(x)) = g(f(x')) = g\circ f(x') = x', \]
	and so $f$ is injective. 
	
	Lastly, suppose $f$ is injective and $g:B\rightarrow A$ is any left-inverse. 
	For any $x\in A$, we have $g(y) = x$ for $y = f(x)$, so $g$ is surjective. 
	
	(b).
	Suppose $f$ is surjective. 
	Let $i:B\rightarrow\cc{P}(A)$ by $i(y) = f^{-1}\{y\}$ and $C = \im i$. 
	To justify existence, apply separation to $B\times_{K}\cc{P}(A)$ to obtain
	\[ G_{i} = \{ o\in B\times_{K}\cc{P}(A) \,|\, Q(o, f) \} \]
	where
	\begin{align*}
		Q(o, c) &\equiv (o\text{ is a Kuratowski ordered pair})\land (c\text{ is a function}) \\
		&\quad\land (\pi_{L}(o)\in\textrm{Cod}(c))\land (\pi_{R}(o) = c^{-1}\{\pi_{L}(o)\}).
	\end{align*}
	Given $G_{i}\subseteq B\times_{K}\cc{P}(A)$, we verify (F1) and (F2) from the definition of functions.
	Here (F1) holds because for all $y\in B$ we have $\langle y, f^{-1}\{y\}\rangle\in G_{i}$, and (F2) holds because if $\langle y, X\rangle, \langle y, X'\rangle\in G_{i}$ then $X = f^{-1}\{y\} = X'$.
	
	By surjectivity of $f$, every inverse image of the form $f^{-1}\{y\}$ for $y\in B$ is nonempty, so every member of $C$ is nonempty.
	By the axiom of choice (ZFC9), there exists a function $j:C\rightarrow\bigcup C$ such that $j(c)\in c$ for all $c\in C$. 
	Note that since $C\subseteq\cc{P}(A)$, we have $\bigcup C\subseteq A$. 
	Then we may form 
	\[ g = j|^{A}\circ i|^{C}:B\rightarrow A. \] 
	For any $y\in B$, we have
	\[ f\circ g(y) = f(g(y)) = f(j(f^{-1}\{y\})) = y \]
	where the last equality follows from $j(f^{-1}\{y\})\in f^{-1}\{y\}$ and $\forall x\, (x\in f^{-1}\{y\}\implies f(x) = y)$. 
	Hence $f\circ g = \id_{B}$ by \Cref{ch1thm_lemma_equality_of_fns}, and $g$ is a right-inverse of $f$.
	
	Conversely, suppose $f$ has a right-inverse $g:B\rightarrow A$. 
	Then for any $y\in B$, we have $f(x) = y$ for $x = g(y)$, so $f$ is surjective.
	
	Lastly, suppose $f$ is surjective and $g:B\rightarrow A$ is any right-inverse. 
	If $y, y'\in B$ such that $g(y) = g(y')$, then $y = f(g(y)) = f(g(y')) = y'$, so $g$ is injective. 
	
	(c).
	Suppose $f$ is bijective, meaning it is both injective and surjective. 
	By (a), $f$ has a left-inverse $g_{L}:B\rightarrow A$, and by (b), $f$ has a right-inverse $g_{R}:B\rightarrow A$. 
	We claim $g_{L} = g_{R}$. 
	For any $y\in B$, applying the left-inverse identity $g_{L}\circ f = \id_{A}$ to $g_{R}(y)$ gives 
	\[ g_{L}(f(g_{R}(y))) = g_{R}(y). \]
	Since $f\circ g_{R} = \id_{B}$, we have $f(g_{R}(y)) = y$, so the left-hand side above is $g_{L}(y)$.
	Thus, $g_{L}(y) = g_{R}(y)$, and since this holds for all $y\in B$, \Cref{ch1thm_lemma_equality_of_fns} gives $g_{L} = g_{R}$. 
	Setting $g = g_{L} = g_{R}$, we find $g\circ f = \id_{A}$ and $f\circ g = \id_{B}$, so $g$ is an inverse of $f$. 
	
	Conversely, suppose $f$ has an inverse $g$. 
	Then $g$ is both a left-inverse and a right-inverse of $f$, so by (a) and (b), $f$ is injective and surjective.
	Thus, $f$ is bijective. 
	
	Lastly, if $g$ is an inverse of $f$, then $f\circ g = \id_{B}$ and $g\circ f = \id_{A}$ show $f$ is an inverse of $g$, so $g$ is invertible. 
	Applying the forward direction just proved to $g$ in place of $f$, we conclude $g$ is bijective.
\end{proof}
...
\begin{definition}
	Let $\langle P, <\rangle$ be a partially ordered set. 
	Suppose $C$ is a subset of $P$ and $<'$ is the restriction of $<$ to $C$.
	If $\langle C, <'\rangle$ is a totally ordered set, we call it a \textbf{chain} of $\langle P, <\rangle$. 
	For brevity, we usually refer to $\langle P, <\rangle$ and $\langle C, <'\rangle$ as just $P$ and $C$. 
\end{definition}

A famous consequence of the axiom of choice involving partial orders is Zorn's lemma.
...
\begin{theorem}[Zorn's lemma]\label{ch1thm_zornslemma}
	Let $\langle P, <\rangle$ be a nonempty partially ordered set.
	If every chain $C$ of $P$ is bounded above, then there exists a maximal element of $P$.
\end{theorem}
\begin{proof}
	Let $\langle P, <\rangle$ be a nonempty partially ordered set and suppose the following:
	\begin{enumerate}[itemsep=-0.8ex, label=(\roman*)]
		\item Every chain of $P$ is bounded above: 
		If $C\subseteq P$ is a chain, there exists $y\in P$ such that $x\le y$ for all $x\in C$. 
		\item There is no maximal element of $P$: 
		If $m\in P$, then there exists $x\in P$ such that $m < x$. 
	\end{enumerate}
	We will show that these two facts lead to a contradiction, so both cannot be true at the same time.
	Thus, if a nonempty partially ordered set satisfies (i), then the set must have a maximal element, which means Zorn's lemma is true. 
	Let $\cc{C}\subseteq\cc{C}(P)$ be the collection of all chains of $P$. 
	For any chain $C$ of $P$, define 
	\[ C_{x} = \{ y\in C \,|\, y < x \}. \]
	
	Suppose $C\subseteq P$ is a chain.
	By fact (i), there exists $m\in P$ such that $x\le m$ for all $x\in C$. 
	By fact (ii), there exists $y\in P$ such that $m < y$.
	Thus, $C$ has a strict upper bound $y$ and we have the following fact:
	\begin{enumerate}[itemsep=-0.8ex, label=(\roman*)]\setcounter{enumi}{2}
		\item Every chain has a strict upper bound.
	\end{enumerate}
	Define $B:\cc{C}\rightarrow\cc{P}(P)$ by
	\[ B(C) = \{ b\in P \,|\, b\text{ is a strict upper bound of } C \}. \]
	By fact (iii), every $B(C)$ is nonempty. 
	Let $\cc{B} = \im B$, which is nonempty since $\varnothing\in\cc{C}$ and $B(\varnothing)\in\cc{B}$. 
	We appeal to the axiom of choice (ZFC9) to invoke a choice function
	\[ f:\cc{B}\rightarrow \bigcup\cc{B} \]
	where $f(B(C))\in B(C)$, i.e. $f(B(C))$ is a strict upper bound of $C$. 
	Compose functions and expand the codomain to get
	\[ g = (f\circ B)|^{P}:\cc{C}\rightarrow P. \]
	Define a chain $C$ to be \textbf{nifty} if the following properties hold:
	\begin{enumerate}[itemsep=-0.3ex, label=(C\arabic*)]
		\item $g(\varnothing)\in C$,
		\item $g(\varnothing)\le x$ for all $x\in C$,
		\item $g(C_{x}) = x$ for all $x\in C$, and
		\item if $X$ is a strict subchain of $C$ such that $X_{x} = C_{x}$ for all $x\in X$, then $g(X)\in C$ and $X = C_{g(X)}$. 
	\end{enumerate}
	Let $\cc{G}$ be the collection of all nifty chains and let $G = \bigcup\cc{G}$.
	We go through and prove the following claims.
	
	\begin{manualthm}{Claim 1}
		For all $C, D\in\cc{G}$, either $C\subseteq D$ or $D\subseteq C$, and $C_{x} = D_{x}$ for all $x\in C\cap D$. 
	\end{manualthm}
	\begin{claimproof}
		Given $C, D\in\cc{G}$, take 
		\[ X = \{ x\in C\cap D \,|\, C_{x} = D_{x} \}. \]
		Since $X\subseteq C\cap D$, we have that
		\[ X\subseteq C, \quad X\subseteq D, \quad\text{and}\quad X\text{ is a chain}. \]
		Suppose $X$ is a proper subset of $C$ and $D$.
		For any $x\in X$, we find $X_{x} = C_{x}$ due to the following reasoning:
		\begin{itemize}[itemsep=-0.0ex]
			\item $(\subseteq).$
			If $y\in X_{x}$, then $y < x$ and $y\in X\subset C$, so clearly $y\in C_{x}$.
			\item $(\supseteq).$
			If $y\in C_{x}$, then $y < x$ and $y\in C$. 
			Now given $C_{x} = D_{x}$, it follows that $y\in D$, and a further restriction of these sets yields $C_{y} = D_{y}$, so $y\in X$ by definition of $X$.
			Given that $y < x$, we have $y\in X_{x}$. 
		\end{itemize}
		Thus, $X$ satisfies the hypothesis of (C4), and because $C$ is nifty, we have $g(X)\in C$ and $X = C_{g(X)}$. 
		By the exact same reasoning, $g(X)\in D$ and $X = D_{g(X)}$. 
		
		Now $g(X)\in C\cap D$ and $C_{g(X)} = D_{g(X)}$, so then $g(X)\in X$ by definition of $X$.
		However, $g(X)$ is a strict upper bound of $X$, so it cannot be contained in $X$.
		This is a contradiction, so the supposition that $X$ is a proper subset of both $C$ and $D$ is false.
		However, $X$ is a subset of $C$ and $D$, so $X = C$ or $X = D$.
		Without loss of generality, assume $X = C$ holds. 
		Given 
		\[ C = X\subseteq C\cap D\subseteq C, \]
		it follows that $C = C\cap D$, which is equivalent to $C\subseteq D$. 
		This proves the first part of Claim 1. 
		The fact that $X = C$ proves the second part of Claim 1.
	\end{claimproof} 
	
	\begin{manualthm}{Claim 2}
		$G$ is a chain.
	\end{manualthm}
	\begin{claimproof}
		The irreflexivity, asymmetry, and transitivity properties of $<$ are trivially passed down from $P$ to $G$.
		It only remains to prove the strict comparability property. 
		Let $x, y\in G$.
		Then $x\in C$ and $y\in D$ for some $C, D\in\cc{G}$. 
		By Claim 1, one of the two sets $C, D$ is a subset of the other, say $C\subseteq D$.
		Then $x, y\in D$ and by the fact that $D$ is a chain it follows that $x < y$, $x = y$, or $x > y$.
		Thus, any two elements of $G$ are comparable, and therefore $G$ is a chain. 
		This proves Claim 2.
	\end{claimproof}
	
	\begin{manualthm}{Claim 3}
		For every $C\in\cc{G}$, we have $C_{x} = G_{x}$ for all $x\in C$.
	\end{manualthm}
	\begin{claimproof}
		Let $C\in\cc{G}$ and $x\in C$.
		Then $C\subseteq G$, and by Claim 2, actually $C$ is a subchain of $G$, so $C_{x}\subseteq G_{x}$.
		It remains to show the opposite inclusion $C_{x}\supseteq G_{x}$. 
		
		Let $y\in G_{x}$, so $y < x$ and there exists $D\in\cc{G}$ such that $y\in D$. 
		By Claim 1, one of the two sets $C, D$ must be a subset of the other.
		If $D\subseteq C$, then $y\in C$ and with $y < x$ we have $y\in C_{x}$. 
		If $C\subseteq D$, then $x\in D$ and by Claim 1 again, we have $C_{x} = D_{x}$ and so $y\in C_{x}$. 
		Thus, $G_{x}\subseteq C_{x}$, and so $C_{x} = G_{x}$, as desired.
		This proves Claim 3.
	\end{claimproof}
	
	\begin{manualthm}{Claim 4}
		$G$ is a nifty chain.
	\end{manualthm}
	\begin{claimproof}
		By Claim 2, $G$ is a chain.
		We will show that $G$ satisfies the properties (C1)--(C4).
		Property (C1) holds, because $\{g(\varnothing)\}\in\cc{G}$.
		Property (C2) holds, because (C2) holds for every $C\in\cc{G}$.
		
		Now we show (C3) applies to $G$.
		Given any $x\in G$, we know $x\in C$ for some $C\in\cc{G}$.
		By Claim 3, $C_{x} = G_{x}$, and since (C3) holds for $C$, we have
		\[ g(G_{x}) = g(C_{x}) = x. \]
		This proves that property (C3) holds.
		
		Lastly, we show (C4) applies to $G$.
		Let $X\subset G$ such that $X_{x} = G_{x}$ for all $x\in X$. 
		Choose any $y\in G\setminus X$. 
		Then $y\in D$ for some $D\in\cc{G}$. 
		If $y < x$ for some $x\in X$, then $y\in G_{x} = X_{x}$ and $y\in X$, which is a contradiction.
		Thus, $y\ge x$ for all $x\in X$, i.e. $y$ is an upper bound of $X$.
		Given that $y\notin X$, we know $y\ne x$ for all $x\in X$, so $y$ is a strict upper bound of $X$.
		
		Since $D_{y} = G_{y}$ (by Claim 3), $X\subset G$, and $y$ is a strict upper bound of $X$, we have
		\[ X\subseteq G_{y} = D_{y}\subset D. \]
		Let $x\in X$. 
		Since $X_{x} = G_{x}$ by assumption and $D_{x} = G_{x}$ by Claim 3, it follows that $X_{x} = D_{x}$.
		Thus, $X\subset D$ and for all $x\in X$ we have $X_{x} = D_{x}$.
		Since (C4) holds for $D$, we have $g(X)\in D$ and $X = D_{g(X)}$. 
		Recalling $D\subseteq G$ (by definition of $G$) and $D_{g(X)} = G_{g(X)}$ (by Claim 3), we conclude $g(X)\in G$ and $X = G_{g(X)}$. 
		This proves that property (C4) holds and completes the proof of Claim 4.
	\end{claimproof}
	
	Now that we've proven that $G$ is a nifty chain, take the strict upper bound $g(G)$ of $G$ and form the set $G\cup\{g(G)\}$.
	
	\begin{manualthm}{Claim 5}
		$G\cup\{g(G)\}$ is a nifty chain.
	\end{manualthm}
	\begin{claimproof}
		Since $G$ is a chain and $g(G)$ is a strict upper bound of $G$, it follows that $G\cup\{g(G)\}$ is a chain.
		We will show that $G\cup\{g(G)\}$ satisfies the properties (C1)--(C4).
		Property (C1) holds, because $g(\varnothing)\in G\subseteq G\cup\{g(G)\}$.
		Property (C2) holds, because (C2) holds for $G$, and $g(\varnothing)\le g(G)$ since $g(G)$ is a strict upper bound of $G$.
		
		Now we show (C3) applies to $G\cup\{g(G)\}$.
		Let $x\in G\cup\{g(G)\}$.
		If $x\in G$, then $(G\cup\{g(G)\})_{x} = G_{x}$, because $g(G)$ is a strict upper bound of $G$.
		Since (C3) holds for $G$, we have
		\[ g((G\cup\{g(G)\})_{x}) = g(G_{x}) = x. \]
		If $x = g(G)$, then $(G\cup\{g(G)\})_{x} = G$, so
		\[ g((G\cup\{g(G)\})_{x}) = g(G) = x. \]
		This proves that property (C3) holds.
		
		Lastly, we show (C4) applies to $G\cup\{g(G)\}$.
		Let $X$ be a strict subchain of $G\cup\{g(G)\}$ such that $X_{x} = (G\cup\{g(G)\})_{x}$ for all $x\in X$.
		If $g(G)\in X$, then the hypothesis $X_{x} = (G\cup\{g(G)\})_{x}$ applies with $x = g(G)$.
		Thus,
		\[ X_{g(G)} = (G\cup\{g(G)\})_{g(G)} = G. \]
		Hence $G\subseteq X$.
		Since $g(G)\in X$, it follows that $G\cup\{g(G)\}\subseteq X$.
		But $X\subseteq G\cup\{g(G)\}$, so $X = G\cup\{g(G)\}$, contradicting the fact that $X$ is a strict subchain.
		Thus, $g(G)\notin X$, so $X\subseteq G$.
		
		If $X = G$, then 
		\[ g(X) = g(G)\in G\cup\{g(G)\} \quad\text{and}\quad X = G = (G\cup\{g(G)\})_{g(G)}. \]
		Thus, property (C4) holds in this case.
		Now suppose instead that $X\subset G$.
		For every $x\in X$, we have
		\[ (G\cup\{g(G)\})_{x} = G_{x}, \]
		because $g(G)$ is a strict upper bound of $G$.
		Hence $X_{x} = G_{x}$ for all $x\in X$.
		Since (C4) holds for $G$, we have $g(X)\in G$ and $X = G_{g(X)}$.
		Note that since $g(X)\in G$ and $g(G)$ is a strict upper bound of $G$, we also have $G_{g(X)} = (G\cup\{g(G)\})_{g(X)}$.
		Therefore,
		\[ g(X)\in G\cup\{g(G)\} \quad\text{and}\quad X = (G\cup\{g(G)\})_{g(X)}. \]
		This proves that property (C4) holds.
		This completes the proof of Claim 5.
	\end{claimproof}
	
	By Claim 5, $G\cup\{g(G)\}$ is a nifty chain, so $G\cup\{g(G)\}\in\cc{G}$.
	Since $G = \bigcup\cc{G}$, we have $G\cup\{g(G)\}\subseteq G$, and thus $g(G)\in G$.
	However, $g(G)$ is a strict upper bound of $G$, so $g(G)\notin G$.
	This is a contradiction, so our initial supposition that facts (i) and (ii) can hold at the same time is false, which is what we set out to prove.
\end{proof}
