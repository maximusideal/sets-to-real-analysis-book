...
We leave it to the reader to show that for any $a, b\in A$ we have $a\sim b$ if and only if $[a] = [b]$, and that $A/\sim\;$ is a partition of $A$. 
...
For any pair $c = \langle a, b\rangle$, let us write $\pi_{L}(c) = a$ and $\pi_{R}(c) = b$.
...
\begin{proof}
	...
	and then form $h = \langle G, \langle X, Y\rangle\rangle$. 
...
% iden map exist and uniqueness

% [comp def'n]

% comp existence and uniqueness

% change of domain and codomain
...
\begin{definition}
	...
	\begin{itemize}[itemsep=-0.4ex]
		\item We say $f$ is \textbf{injective} if for all $a, b\in A$ we have $f(a) = f(b)\implies a=b$.
		\item We say $f$ is \textbf{surjective} if for all $b\in B$ there exists $a\in A$ such that $f(a) = b$.
...
\begin{exer}
	Explain why the following definitions are equivalent to the ones above: $f$ is injective if for all $a, b\in A$ we have $a\ne b\implies f(a)\ne f(b)$, and $f$ is surjective if $\im f = B$.
\end{exer}

% defin of inverse maps (left right both)


\subsection{Functions and Quotients}

Let us revisit the notion of quotient sets.
...
for each $a\in A$. 
In mathematics, we often want to define functions on quotient sets.
That is, we are often interested in defining functions of the form $f:A/\sim\;\rightarrow B$. 
However, we have to be careful:
If we define $f([a])$ by appealing to properties of the representative $a$, we must make sure that the value of $f([a])$ is the same no matter which representative of the class $[a]$ we choose. 
In other words, if $[a] = [a']$, we must have $f([a]) = f([a'])$.
If this does not hold, then the function is \textbf{not well-defined}. 
When something is not well-defined, there is a lack of clarity or precision in the formulation of the object.

\begin{example}[$f$ is not well-defined]\label{ch1ex_f_not_well_defd}
	Let $A = \{ a, b, c, d, i, j \}$ and $B = \{ x, y \}$ where all elements listed are distinct from one another.
	Let $C = \{ a, c, i \}$ as well. 
	Define a relation $\sim$ on $A$ by
	\begin{align*}
		a\sim a, \quad b\sim b, \quad c\sim c, \quad d\sim d, \quad i\sim i, \quad j\sim j, \\
		a\sim b, \quad b\sim a, \quad c\sim d, \quad d\sim c, \quad i\sim j, \quad j\sim i.
	\end{align*}
	We leave it to the reader to check this is an equivalence relation on $A$.
	The resulting quotient set is $A/\sim\; = \{\{a, b\}, \{c, d\}, \{i, j\}\}$. 
	...
	\begin{align*}
		...
		\end{cases}
	\end{align*}
	\emph{This function is not well-defined}.
	The problem is that because $[a] = [b]$, it must be the case that $f([a]) = f([b])$, but $f([a]) = x$ and $f([b]) = y$.
	By the substitution property of equality, we have $x = y$, but we declared at the start that $x$ and $y$ are distinct elements, so we end up in a contradiction. 
\end{example}

Just because we provided a description of a function $f$, it does not automatically follow that $f$ is well-defined. 
\Cref{ch1ex_f_not_well_defd} is a good demonstration of why it helps to have a formal theory such as ZFC to fall back on when we get into trouble. 
...
\begin{lemma}
	If $\sim$ is an equivalence relation on a set $A$, then the quotient map $q:A\rightarrow A/\sim$ exists and is surjective.
\end{lemma}
\begin{proof}
	Given $A$ and $\sim\; = ((A, A)_{K}, G_{\sim})_{K}$, recall that the axiom schema of separation (ZFC5) justifies the existence of equivalence classes $[a] = \{ a'\in A \,|\, a\sim a' \}$. 
	The axiom of power set (ZFC6) and the axiom schema of separation (ZFC5) then justify the existence of $Q = A/\sim$. 
	By \Cref{ch1thm_kuratowski_prod_exists}, $A\times_{K} Q$ exists. 
	By the axiom schema of separation (ZFC5), the set
	\[ G = \{ x\in A\times_{K} Q \,|\, x = (a, [a])_{K} \text{ for some } a\in A \} \]
	exists. 
	By \Cref{ch1thm_kuratowski_pair_exist}, we construct $q = ((A, Q)_{K}, G)_{K}$. 
	
	We will show $q$ is a function. 
	First, note that $G\subseteq A\times_{K} Q$. 
	Second, if $a\in A$, then $[a]\in Q$ such that $(a, [a])_{K}\in G$. 
	Thus, (F1) holds. 
	Third, if $a\in A$ and $b, b'\in Q$ such that $(a, b)_{K}, (a, b')_{K}\in G$, then by definition of $G$, $b = [a]$ and $b' = [a]$, so $b = b'$. 
	Thus, (F2) holds. 
	
	It remains to show $q$ is surjective.
	By definition, if $x\in A/\sim$ then $x = [a]$ for some $a\in A$, so $q(a) = x$. 
	This completes the proof. 
\end{proof}

\begin{theorem}[Quotient factorization theorem for sets]\label{ch1thm_quotient_factor_thm}
	Suppose $\sim$ is an equivalence relation on a set $A$ and $f:A\rightarrow B$ is a function such that $a\sim a'$ implies $f(a) = f(a')$. 
...
The proof of \Cref{ch1thm_quotient_factor_thm} is given as optional reading in \Cref{chA_set_theory_pfs}.
\Cref{ch1thm_quotient_factor_thm} is exactly what we need to guarantee that any function we define on quotient sets is well-defined. 
Let $A, B$ be sets and $\sim$ be an equivalence relation on $A$.
To define a function $g:A/\sim\;\rightarrow B$ so that $g([a])$ is defined by an appeal to the representative $a$ of $[a]$, it suffices to proceed as follows:
\begin{enumerate}[itemsep=-0.4ex, label=(\arabic*)]
	\item Think about how you want to define $g$.
	Given an equivalence class $[a]$, there is some property of the representative $a$ that you want to use to define $g([a])$. 
	Define the function $f:A\rightarrow B$ in which $f(a)$ is defined that property of $a$. 
	\item Check whether 
	\begin{align}\label{ch1eqn_sim_implication_fn}
		a\sim a'\implies f(a) = f(a')
	\end{align}
	for all $a, a'\in A$. 
	If \Cref{ch1eqn_sim_implication_fn} does not hold, then the desired function $g$ cannot be constructed.  
	If \Cref{ch1eqn_sim_implication_fn} holds, proceed to the next step. 
	\item Apply \Cref{ch1thm_quotient_factor_thm} to obtain a unique function $g:A/\sim\;\rightarrow B$ such that $g([a]) = f(a)$. 
	This is the desired function. 
\end{enumerate}
It is common to define $g:A/\sim\;\rightarrow B$ directly (as in \Cref{ch1ex_f_not_well_defd}) and verify \textit{a posteriori} whether $g$ is well-defined. 
While this approach is often harmless in practice, it can obscure the underlying logic and give the impression of ad hoc justification. 
Here, we have presented a more systematic account of how functions on quotient sets may be defined.
