...
\begin{proof}
	...
	\end{enumerate}
	Define 
	\[ B:\cc{C}\rightarrow\cc{P}(P) \quad\text{by}\quad B(C) = \{ b\in P \,|\, b\text{ is a strict upper bound of } C \}. \]
	...
	\[ f:\cc{B}\rightarrow \bigcup\cc{B} \quad\text{where}\quad f(B(C))\in B(C), \]
	i.e. $f(B(C))$ is a strict upper bound of $C$. 
	Change codomains and compose functions to get
	\[ g = f|^{P}\circ B|^{\cc{B}}:\cc{C}\rightarrow P. \]
	Define a chain $C$ to be \textbf{nifty} if the following properties hold:
	\begin{enumerate}[itemsep=-0.3ex, label=(N\arabic*)]
	...
	\end{enumerate}
	Let $\cc{N}$ be the collection of all nifty chains and let $N = \bigcup\cc{N}$.
	We go through and prove the following claims.
	
	\begin{manualthm}{Claim 1}
		For all $C, D\in\cc{N}$, either $C\subseteq D$ or $D\subseteq C$, and $C_{x} = D_{x}$ for all $x\in C\cap D$. 
	\end{manualthm}
	\begin{claimproof}
		Given $C, D\in\cc{N}$, take 
		...
		\end{itemize}
		Thus, $X$ satisfies the hypothesis of (N4), and because $C$ is nifty, we have $g(X)\in C$ and $X = C_{g(X)}$. 
	...
	\end{manualthm}
	\begin{claimproof}
		The irreflexivity, asymmetry, and transitivity properties of $<$ are trivially passed down from $P$ to $N$.
		It only remains to prove the strict comparability property. 
		Let $x, y\in N$.
		Then $x\in C$ and $y\in D$ for some $C, D\in\cc{N}$. 
		...
		Thus, any two elements of $N$ are comparable, and therefore $N$ is a chain. 
		This proves Claim 2.
	\end{claimproof}
	
	\begin{manualthm}{Claim 3}
		For every $C\in\cc{N}$, we have $C_{x} = N_{x}$ for all $x\in C$.
	\end{manualthm}
	\begin{claimproof}
		Let $C\in\cc{N}$ and $x\in C$.
		Then $C\subseteq N$, and by Claim 2, actually $C$ is a subchain of $N$, so $C_{x}\subseteq N_{x}$.
		It remains to show the opposite inclusion $C_{x}\supseteq N_{x}$. 
		
		Let $y\in N_{x}$, so $y < x$ and there exists $D\in\cc{N}$ such that $y\in D$. 
		...
		Thus, $N_{x}\subseteq C_{x}$, and so $C_{x} = N_{x}$, as desired.
		This proves Claim 3.
	\end{claimproof}
	...
	\end{manualthm}
	\begin{claimproof}
		By Claim 2, $N$ is a chain.
		We will show that $N$ satisfies the properties (N1)--(N4).
		Property (N1) holds, because $\{g(\varnothing)\}\in\cc{N}$.
		Property (N2) holds, because (N2) holds for every $C\in\cc{N}$.
		
		Now we show (N3) applies to $N$.
		Given any $x\in N$, we know $x\in C$ for some $C\in\cc{N}$.
		By Claim 3, $C_{x} = N_{x}$, and since (N3) holds for $C$, we have
		\[ g(N_{x}) = g(C_{x}) = x. \]
		This proves that property (N3) holds.
		
		Lastly, we show (N4) applies to $N$.
		Let $X\subset N$ such that $X_{x} = N_{x}$ for all $x\in X$. 
		Choose any $y\in N\setminus X$. 
		Then $y\in D$ for some $D\in\cc{N}$. 
		If $y < x$ for some $x\in X$, then $y\in N_{x} = X_{x}$ and $y\in X$, which is a contradiction.
		Thus, $y\ge x$ for all $x\in X$, i.e. $y$ is an upper bound of $X$.
		Given that $y\notin X$, we know $y\ne x$ for all $x\in X$, so $y$ is a strict upper bound of $X$.
		
		Since $D_{y} = N_{y}$ (by Claim 3), $X\subset N$, and $y$ is a strict upper bound of $X$, we have
		\[ X\subseteq N_{y} = D_{y}\subset D. \]
		Let $x\in X$. 
		Since $X_{x} = N_{x}$ by assumption and $D_{x} = N_{x}$ by Claim 3, it follows that $X_{x} = D_{x}$.
		Thus, $X\subset D$ and for all $x\in X$ we have $X_{x} = D_{x}$.
		Since (N4) holds for $D$, we have $g(X)\in D$ and $X = D_{g(X)}$. 
		Recalling $D\subseteq N$ (by definition of $N$) and $D_{g(X)} = N_{g(X)}$ (by Claim 3), we conclude $g(X)\in N$ and $X = N_{g(X)}$. 
		This proves that property (N4) holds and completes the proof of Claim 4.
	\end{claimproof}
	...
	\begin{manualthm}{Claim 5}
		$N\cup\{g(N)\}$ is a nifty chain.
	\end{manualthm}
	\begin{claimproof}
		Since $N$ is a chain and $g(N)$ is a strict upper bound of $N$, it follows that $N\cup\{g(N)\}$ is a chain.
		We will show that $N\cup\{g(N)\}$ satisfies the properties (N1)--(N4).
		Property (N1) holds, because $g(\varnothing)\in N\subseteq N\cup\{g(N)\}$.
		Property (N2) holds, because (N2) holds for $N$, and $g(\varnothing)\le g(N)$ since $g(N)$ is a strict upper bound of $N$.
		
		Now we show (N3) applies to $N\cup\{g(N)\}$.
		Let $x\in N\cup\{g(N)\}$.
		If $x\in N$, then $(N\cup\{g(N)\})_{x} = N_{x}$, because $g(N)$ is a strict upper bound of $N$.
		Since (N3) holds for $N$, we have
		\[ g((N\cup\{g(N)\})_{x}) = g(N_{x}) = x. \]
		If $x = g(N)$, then $(N\cup\{g(N)\})_{x} = N$, so
		\[ g((N\cup\{g(N)\})_{x}) = g(N) = x. \]
		This proves that property (N3) holds.
		
		Lastly, we show (N4) applies to $N\cup\{g(N)\}$.
		Let $X$ be a strict subchain of $N\cup\{g(N)\}$ such that $X_{x} = (N\cup\{g(N)\})_{x}$ for all $x\in X$.
		If $g(N)\in X$, then the hypothesis $X_{x} = (N\cup\{g(N)\})_{x}$ applies with $x = g(N)$.
		Thus,
		\[ X_{g(N)} = (N\cup\{g(N)\})_{g(N)} = N. \]
		Hence $N\subseteq X$.
		Since $g(N)\in X$, it follows that $N\cup\{g(N)\}\subseteq X$.
		But $X\subseteq N\cup\{g(N)\}$, so $X = N\cup\{g(N)\}$, contradicting the fact that $X$ is a strict subchain.
		Thus, $g(N)\notin X$, so $X\subseteq N$.
		
		If $X = N$, then 
		\[ g(X) = g(N)\in N\cup\{g(N)\} \quad\text{and}\quad X = N = (N\cup\{g(N)\})_{g(N)}. \]
		Thus, property (N4) holds in this case.
		Now suppose instead that $X\subset N$.
		For every $x\in X$, we have
		\[ (N\cup\{g(N)\})_{x} = N_{x}, \]
		because $g(N)$ is a strict upper bound of $N$.
		Hence $X_{x} = N_{x}$ for all $x\in X$.
		Since (N4) holds for $N$, we have $g(X)\in N$ and $X = N_{g(X)}$.
		Note that since $g(X)\in N$ and $g(N)$ is a strict upper bound of $N$, we also have $N_{g(X)} = (N\cup\{g(N)\})_{g(X)}$.
		Therefore,
		\[ g(X)\in N\cup\{g(N)\} \quad\text{and}\quad X = (N\cup\{g(N)\})_{g(X)}. \]
		This proves that property (N4) holds.
		This completes the proof of Claim 5.
	\end{claimproof}
	
	By Claim 5, $N\cup\{g(N)\}$ is a nifty chain, so $N\cup\{g(N)\}\in\cc{N}$.
	Since $N = \bigcup\cc{N}$, we have $N\cup\{g(N)\}\subseteq N$, and thus $g(N)\in N$.
	However, $g(N)$ is a strict upper bound of $N$, so $g(N)\notin N$.
